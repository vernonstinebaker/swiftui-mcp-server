...
- `ForEach` over an `enumerated()` sequence should not convert to an array first. Use `ForEach(items.enumerated(), id: \.
  element.id)` directly.
- When hiding scroll indicators, use `.scrollIndicators(.hidden)` rather than `showsIndicators: false` in the initializer.
- Never use `Text` concatenation with `+`. Instead, use text interpolation like this: let red = Text("Hello").foregroundStyle(.red); let blue = Text("World").foregroundStyle(.blue); Text("\(red)\(blue)")
